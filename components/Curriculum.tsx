...
const Curriculum: React.FC<CurriculumProps> = ({ onNavigateToBuild }) => {
  const [currentStep, setCurrentStep] = useState(0);
  const [userIdea, setUserIdea] = useState('');
  const [selectedTool, setSelectedTool] = useState<string | null>(null);
  const [generatedPrompt, setGeneratedPrompt] = useState('');
  const [isGenerating, setIsGenerating] = useState(false);
  const [hfStatus, setHfStatus] = useState<'idle' | 'loading' | 'success' | 'fallback'>('idle');
        ...
        return (
          <div className="animate-fade-in text-center py-8">
            ...
            <div className="bg-brand-gray border border-white/10 rounded-xl p-8 max-w-2xl mx-auto mb-12 text-left">
              <h4 className="text-brand-primary font-bold uppercase tracking-widest text-xs mb-4 flex items-center gap-2">
              ...
              <div className="mb-4 flex items-center gap-2 text-xs">
                {hfStatus === 'loading' && (
                  <span className="inline-flex items-center gap-1.5 text-blue-400 bg-blue-500/10 border border-blue-500/20 px-3 py-1.5 rounded-full">
                    <Loader2 className="w-3 h-3 animate-spin" /> Generating with HuggingFace Mistral-7B...
                  </span>
                )}
                {hfStatus === 'success' && (
                  <span className="inline-flex items-center gap-1.5 text-green-400 bg-green-500/10 border border-green-500/20 px-3 py-1.5 rounded-full">
                    <Cloud className="w-3 h-3" /> Generated by HuggingFace Mistral-7B
                  </span>
                )}
                {hfStatus === 'fallback' && (
                  <span className="inline-flex items-center gap-1.5 text-yellow-400 bg-yellow-500/10 border border-yellow-500/20 px-3 py-1.5 rounded-full">
                    <AlertCircle className="w-3 h-3" /> Using starter prompt — HuggingFace model warming up
                  </span>
                )}
              </div>
